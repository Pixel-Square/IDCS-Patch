...
def _jwt_authenticate_request(request):
    ...
    # Browser top-level navigations cannot attach Authorization headers.
    # Support a query-token fallback for OAuth-start URLs.
    if not res:
        qs_token = (
            str(request.GET.get('token', '') or '').strip()
            or str(request.GET.get('access_token', '') or '').strip()
            or str(request.GET.get('jwt', '') or '').strip()
        )
        if qs_token:
            ...
            # If framework auth still fails, validate token claims manually.
            if not res:
                try:
                    token_obj = AccessToken(qs_token)
                    user_id = token_obj.get(jwt_api_settings.USER_ID_CLAIM)
                    if user_id is not None:
                        UserModel = get_user_model()
                        lookup = {jwt_api_settings.USER_ID_FIELD: user_id}
                        user = UserModel.objects.filter(**lookup).first()
                        if user and jwt_api_settings.USER_AUTHENTICATION_RULE(user):
                            res = (user, token_obj)
                except Exception:
                    res = None
