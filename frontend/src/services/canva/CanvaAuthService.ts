...
export function initiateOAuth(): void {
  ...
  const directAuthorizeUrl = token
    ? `/api/canva/oauth/authorize?origin=${origin}&token=${encodeURIComponent(token)}`
    : `/api/canva/oauth/authorize?origin=${origin}`;
  ...
  void (async () => {
    try {
      ...
      if (!res.ok || !data.authorize_url) {
        // Fallback: direct browser navigation using query-token auth.
        if (token) {
          window.location.href = directAuthorizeUrl;
          return;
        }
